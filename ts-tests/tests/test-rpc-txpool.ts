...
import { createErc20Context, erc20BalanceOf, erc20Transfer } from "./utils-erc20";
...
describeWithFrontier("ETH RPC(Txpool For ERC20)", (context) => {
	...
	before("create the contract", async function () {
		this.timeout(15000);
		CONTRACT_ADDRESS = await createErc20Context(context.web3, GENESIS_ACCOUNT, GENESIS_ACCOUNT_PRIVATE_KEY);
	});
	...
	step("Get txpool content rpc", async function () {
		...
		expect(await erc20BalanceOf(context.web3, CONTRACT_ADDRESS, TEST_ACCOUNT)).to.be.eq("3000");
